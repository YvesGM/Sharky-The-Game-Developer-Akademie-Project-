...
export default class SharkyClass extends MovableObjectsClass {
    ...
    getAttackBox(camera_x = 0) {
        ...
        let attackRange = this.attackType === 'bubble' ? this.w * 2.15 : 190;
        let attackHeightOffset = this.attackType === 'bubble' ? 10 : 5;

        if (this.otherDirection) {
            return {
                x: hitbox.x - attackRange,
                y: hitbox.y + attackHeightOffset,
                w: attackRange,
                h: hitbox.h - attackHeightOffset * 2
            };
        }
